...
```
OPTIONS="-i <interface1> -i <interface2> -E -2 -H -l 6 -f /etc/ptp4l.conf"
```

Create the configuration file for phc2sys (`/etc/sysconfig/phc2sys`)

```
OPTIONS="-s <inteface1> -w -R 8 "
```
...
### (Re-)create VF(s)
```bash
# Use the vf_setup_du.sh script from ios-mcn-ran folder to configure the VFs
cd ./scripts
sudo ./vf_setup_du.sh --interface <IF_NAME> --vlan <VLAN> --mac1 <MAC_1> --mac2 <MAC_2> --driver vfio-pci 
```

The `vf_setup_du.sh` script automates the following VF configuration:
- Creates virtual functions on the physical interface
- Assigns MAC addresses to each VF
- Configures VLAN tagging on the VFs
- Sets MTU size appropriately
- Binds the VFs to DPDK using `dpdk-devbind.py`

After running the script, verify the VF PCI addresses with:
```bash
lspci | grep Virtual
```

Use these PCI addresses in the `fhi_72.dpdk_devices` parameter of your DU configuration file.
