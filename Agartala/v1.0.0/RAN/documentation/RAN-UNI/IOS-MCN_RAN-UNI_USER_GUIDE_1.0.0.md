# IOS MCN v1.0.0 Agartala Release - User Guide 
-----
The **IOS-MCN RAN Sub-Project** aim to deliver a functional, high-performance, and easily deployable Radio Access Network (RAN) based on open-source components such as *OpenAirInterface5G (OAI)*. This user guide outlines the complete process required to deploy a working 5G gNB (gNodeB) capable of interacting with either a LPRU/HPRU or software-defined radio hardware like USRP B210.
...
**4.1. PEGATRON Configuration/Setup:**
 
*To access this Pegatron HPRU (HIGH POWER RADIO UNIT)*
  
We need to login via browser GUI into the machine with the below provided details at IP Address - * 192.168.120.x* (default ip provided by the RRU supplier)

- Username - *admin*
- Password: *the password set by the developer*

![](../../documentation/RAN-UNI/images/ran_installation/.png)

Figure#: Logging into RRU GUI(localhost)

Once you are logged in, Follow the below provided steps to sync up the HPRU, this includes the synchronization of radio unit, the compression setting , RF antenna configuration, channel parameters and other things.

**1. To track synchronization progress in real-time by filtering sync-related log entries until the system achieves synchronization. Run command:**

![](../../documentation/RAN-UNI/images/ran_installation/HPRU_port_offline.png)
...
>- On the GUI , the PTP locked status looks like the example given below:

![](../../documentation/RAN-UNI/images/ran_installation/HPRU_port_locked.png)

**3. To check and load the clock profile for time synchronization applying system or test setup parameters.Apply the values from GUI:**

![](../../documentation/RAN-UNI/images/ran_installation/clockconfiguration.png)
 
*Depending on whether we are running SISO (1x1) or MIMO (2x2) and e/f Fronthaul Interface*

![](../../documentation/RAN-UNI/images/ran_installation/Settings.png)

Check the RF Configuration:

![](../../documentation/RAN-UNI/images/ran_installation/RFconfiguration_Settings.png)

**3. To parse and load the 2x2 for applying system or test setup parameters.Apply the values from GUI:**
...
```
cd <v.1.0.0.baremetal.tar.gz> #go the directory where the RAN-source package was unzipped and built.

sudo taskset -c 10 ./nr-softmodem -O ./<configuration file> --sa -reorder-thread-disable 1 --thread-pool 3,4,5,7  #3,4,5,7 are the cores used IN CORE ISOLATION

```
**5. UE (User Equipment/Mobile Phones)**
---
...
**6.1.1.1 Core Network Status Check**
...
**6.1.1.2. Start the RAN(Radio Access Network) Check**
...
**6.1.1.3 SIM Card Configuration**
...
**6.1.1.4. Android UE Settings - UE1**
...
**6.1.1.5. Android UE Settings - UE2**
...
**6.1.1.6. UE1 Registration Process**
...
**6.1.1.7. UE2 Registration Process**
...
**6.1.1.8. Verify VoNR is Active on Both UEs On each UE**
...
**6.1.2.1. Outgoing Call - UE1 to UE2**
...
**6.1.2.2. Call Progress Monitoring**
...
**6.1.2.3. Bidirectional Call Testing**
...
**6.1.2.4. Incoming Call - UE2 to UE1**
...
## 6.2. Intra-frequency N2 Handover:

  **6.2.1. Configuration/Setup**
...
**6.2.1.1. Core Network Status Check**
- Verify AMF (Access and Mobility Management Function) is running
- Confirm SMF (Session Management Function) is operational
- Check UPF (User Plane Function) connectivity
- Confirm the IMSI of the SIM's insterted has been added to the HSS in the core

**6.2.1.2. Start the RAN(Radio Access Network) Check**
- Verify gNodeB is broadcasting 5G NR signals
- Confirm cell parameters (PCI, TAC, PLMN)
...
**6.2.1.3 SIM Card Configuration**
- Verify SIM cards have correct IMSI, Ki, OPc values

**6.2.1.4. Android UE Settings - UE1**
1. Power on the first Android UE
2. Navigate to **Settings > Mobile Network**
3. Select your carrier/SIM card
4. Enable **5G** network mode
...
**6.2.1.5. Android UE Settings - UE2**
Repeat the same configuration steps as UE1:
1. Power on the second Android UE
2. Configure identical network and call settings
3. Ensure both UEs use the same network operator settings

**6.2.1.6. UE1 Registration Process**
1. Verify UE1 shows **5G** or **5G SA** in status bar
2. Check signal strength is adequate (-70 dBm or better)
3. Confirm UE has obtained IP address via PDU session
4. Monitor registration logs for successful AMF attachment
...
**6.2.1.7. UE2 Registration Process** (optional, to be used if using 2 UE in the test cases)
1. Repeat registration verification for UE2
2. Ensure both UEs are registered to same AMF/cell
3. Verify both UEs have active PDU sessions
   

**6.2.1.8. Verify the signal is Active on Both UEs**
...
**6.2.2.1. Running the active ping**
...
**6.2.2.2. Powering on the Target gNB**
...
**6.2.2.3. Performing the Handover**
...
**6.2.2.4. Repeating the same process with UE2**
...
**6.3.2.1. Core Network Status Check**
- Verify AMF (Access and Mobility Management Function) is running
- Confirm SMF (Session Management Function) is operational
- Check UPF (User Plane Function) connectivity


**6.3.2.2. Start the RAN(Radio Access Network) Check**
- Verify gNodeB is broadcasting 5G NR signals
- Confirm cell parameters (PCI, TAC, PLMN)
- Check the parameters described in the description of this section are configured in the configuration file


**6.3.2.3 SIM Card Configuration**
- Insert SIM cards with IMS provisioning in both UEs
- Verify SIM cards have correct IMSI, Ki, OPc values
- Confirm IMS APN settings are provisioned on SIM


**6.3.2.4. Android UE Settings - UE1**
1. Power on the first Android UE
2. Navigate to **Settings > Mobile Network**
3. Select your carrier/SIM card
4. Enable **5G** network mode
...
**6.3.2.5. Android UE Settings - UE2**
Repeat the same configuration steps as UE1:
1. Power on the second Android UE
2. Configure identical network and call settings
3. Ensure both UEs use the same network operator settings
...
|---------------------|---------|-------|
| Integration Guide   |  To integration with Radio unit and Core|https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_INTEGRATION_GUIDE_1.0.0.md   |
| Developers Guide    | To build UNI RAN in Different environment       | https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_DEVELOPER_GUIDE_1.0.0.md |
|Installation Guide  |  To install the UNI RAN environment    | https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_INSTALLATION_GUIDE_1.0.0.md    |
|Troubleshooting Guide|To help the user troubleshoot issues   | https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_TROUBLESHOOTING_GUIDE_1.0.0.md|
