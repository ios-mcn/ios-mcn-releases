...
>- 6.4 **Pegatron HPRU CONFIGURATION**
...
The **IOS-MCN RAN Uni Sub-Project** aims to deliver a
...
-   **Radio Hardware:**
    ...
    -   Lekha RU ( for split 7.2)
    -   Pegatron(for split 7.2)
...
----
 ...
 Download the tar file from the Official channel IOSMCN-RAN/RELEASE-IMAGES (https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/release-images/RAN-UNI/ios-mcn-ran-uni-baremetal-v1.0.0.tar.gz)
...
**Untar the file using cmd.**
 ...
 ```   
tar -xvzf ios-mcn-ran-uni-baremetal-v1.0.0.tar.gz
cd ios-mcn-ran

 ```   
**NOTE: Please ensure the tar file must have all the contents as shown below.**

-If the tar package contains the files in the 'bin','lib' and 'utils' directories then you need to move all of them under the directoy 'ios-mcn-ran/build"
...
> **Example command:**
```
sudo patchelf --set-rpath ~/ios-mcn-ran/ nr-softmodem
```
...
```
**CHAPTER-6 RUNNING THE RAN APPLICATION**
=====
...
(wget https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/release-images/RAN-UNI/ios-mcn-ran-uni-baremetal-v1.0.0.tar.gz)

Copy the tar file to the target gNB machine (/home/$user/) .Untar the file using the command given below.
...
```
cd ios-mcn-ran
sudo taskset -c 10 ./nr-softmodem -O ./<configuration file> --sa -E --usrp-tx-thread-config 1 --thread-pool 3,4,5   #conf file cand be found in the Ran.source package under the openairinterfacev1.0.0/ci-scripts 
```
...
sudo taskset -c 10 ./nr-softmodem -O ./<configuration file> --sa -reorder-thread-disable 1 --thread-pool 3,4,5,7  #3,4,5,7 are the cores used in isolation
```
...
Logging into RRU GUI(localhost)
...
```
sleep 1
taskset -c 2 ptp4l -i <port-name> -m -2 -f /home/IOS-MCN-RAN-UNI/linuxptp-master/default.cfg
```
The default clock configuration is as follows:
...
-> For synchronization of the master clock with the hardware we use the below given script:

```
#!/bin/bash
echo "Running phc2sys on gNB(master) for clock profile G.8275.1.."
sleep 1
taskset -c 2 phc2sys -s <port-name> -m -O -37
...
![](../../documentation/RAN-UNI/images/ran_installation/frequency.png)
...
```
sudo docker pull ghcr.io/ios-mcn/ran-gnb-uni:v1.0.0
```
> **8.1.2.** **Pulling the docker image for Split 7.2 architecture** 
```
sudo docker pull ghcr.io/ios-mcn/ran-gnb-fhi72-uni:v1.0.0
```
