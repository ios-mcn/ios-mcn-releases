# IOS-MCN  RAN DEVELOPER GUIDE V1.0.0
...
```
wget https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/source-code/iosmcn.agartala.v1.0.0.RAN.source.tar
```

Step 3: Untar the iosmcn.agartala.v1.0.0.ran.source.tar

```
tar -xvf iosmcn.agartala.v1.0.0.ran.source.tar
cd iosmcn.agartala.v1.0.0.RAN.source.tar

tar -xvzf openairinterface5g-1.0.0.openairinterface5g.tar.gz
cd openairinterface5g-1.0.0.openairinterface5g
```
...
**Compiled IOSMCN-RAN 5G RAN and PHY libraries, including core, PHY, MAC, LDPC, and fronthaul components.**
...
```
**Apply patches  (available in openairinterface5g-1.0.0.openairinterface5g/cmake_targets/tools/oran_fhi_integration_patches/E)**

```
git apply openairinterface5g-1.0.0.openairinterface5g/cmake_targets/tools/oran_fhi_integration_patches/E/oaioran_E.patch
```
```
cd ~/phy/fhi_lib/lib
```
...
You can now proceed building IOSMCN-RAN. You build it the same way as for other radios, providing option **-t oran_fhlib_5g_mplane**
Additionally, you need to provide it the location of the FH library: **--cmake-opt -Dxran_LOCATION=PATH**. 


```
cd ~/IOSMCN-RAN-Source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/cmake_targets
```
...
```
sudo ./build_oai --gNB --ninja --build-lib telnetsrv -t oran_fhlib_5g_mplane --cmake-opt -DOAI_FHI72_USE_POLLING=ON --cmake-opt -DUNI_RAN=ON --cmake-opt -Dxran_LOCATION=(complete path to phy)/fhi_lib/lib
```

Artifact for the build will be found at **( ~/IOSMCN-RAN-Source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/cmake_targets/ran_build/build/nr-softmodem)** The final artifact includes the shared libaries build along with nr-softmodem and the libxran library. Here are the details for the libraries.
...
**NOTE**- The above mentioned library files denoted with asterisk are used for the Telnet Server in IOSMCN-RAN is an embedded command-line interface (CLI) tool that allows developers and testers to interact with the running softmodem (e.g., gNB, eNB, UE) for debugging, monitoring, and configuration purposes. These libraries are dynamically loaded based on the executable being run.
...
```
cd ~/IOSMCN-RAN-Source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/common/utils/T/tracer
```
```
make all
```
This utility also needs a database file (T_messages.txt) which is also the part of tar package. You can get that file from(~/IOSMCN-RAN-Source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/common/utils/T).


# V. DOCKERIZED RAN BUILD GUIDE

This Guide is intended to use to build the docker images for the ran source package
**iosmcn.agartala.v1.0.0.ran.source.tar**

```
cd ~/IOSMCN-RAN-source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/docker/
```

**Follow the instructions in the README.md file present at this path [~/IOSMCN-RAN-source/iosmcn.agartala.v1.0.0.RAN.source.tar/openairinterface5g-1.0.0.openairinterface5g/docker/README.md]**
...
|---------------------|---------|-------|
| Integration Guide   |  To integrate the gNB with Radio unit and Core| https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_INTEGRATION_GUIDE_1.0.0.md |
|User Guide    | To use RAN-UNI with test environment       | https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_USER_GUIDE_1.0.0.md       |
...
|Troubleshooting Guide|To help the user troubleshoot issues         | https://github.com/ios-mcn/ios-mcn-releases/tree/main/Agartala/v1.0.0/RAN/documentation/RAN-UNI/IOS-MCN_RAN-UNI_TROUBLESHOOTING_GUIDE_1.0.0.md  |
  ...
  **Any specific support on IOSMCN-RAN, please raise issues at IOS-MCN-Releases**(https://github.com/ios-mcn/ios-mcn-releases/issues)
